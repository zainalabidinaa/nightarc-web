...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    if (recommendations?.rows && recommendations.rows.length > 0) return;
    if (autoGeneratedRef.current) return;
    const hasHistory = (initialData?.progress?.length ?? 0) > 0;
    if (!hasHistory) return;
    autoGeneratedRef.current = true;
    recMutation.mutate();
  }, [recommendations?.rows, initialData?.progress, currentProfile?.id]);
