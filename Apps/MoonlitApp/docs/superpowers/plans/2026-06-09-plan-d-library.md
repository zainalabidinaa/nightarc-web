...
**Architecture:** `LibraryScreen.swift` becomes a single `ScrollView` with three `LibrarySection` views. A new `UpcomingItemsService` in MoonlitCore classifies liked items into released vs upcoming using TMDB `release_date` / `next_episode_to_air` / `status` fields. `LikedRepository` (created in Plan C) is the source of truth for liked items. `LibraryRepository` remains for watchlist items.

**Tech Stack:** SwiftUI, MoonlitCore, TMDB REST API, BackgroundTasks, UserDefaults
...
| Action | Path |
|---|---|
| Create | `Packages/MoonlitCore/Sources/MoonlitCore/Services/UpcomingItemsService.swift` |
...
```swift
// Packages/MoonlitCore/Sources/MoonlitCore/Services/UpcomingItemsService.swift
import Foundation
...
```bash
git add Packages/MoonlitCore/Sources/MoonlitCore/Services/UpcomingItemsService.swift
...
grep -n "struct LibraryScreen\|LazyVGrid\|libraryItems\|BookmarkItem" \
  /Users/zain/projects/Moonlit/Apps/MoonlitApp/Sources/Screens/LibraryScreen.swift | head -20
```
                ...
                ForEach(MediaFilter.allCases, id: \.self) { filter in
                    Button {
                        ...
                        Text(filter.rawValue)
                            ...
                            .padding(.vertical, 5)
                            .background(selection.wrappedValue == filter ? MoonlitTheme.accent : Color.white.opacity(0.08))
                            .cornerRadius(20)
                    }
                }
            }
...
> ```bash
> grep -n "struct DetailScreen\|init(" Apps/MoonlitApp/Sources/Screens/DetailScreen.swift | head -10
> ```

- [ ] **Step 3: Build**

```bash
xcodebuild -scheme MoonlitApp -destination 'platform=iOS Simulator,name=iPhone 16' build 2>&1 | grep -E "error:|Build succeeded|Build FAILED" | tail -30
```
...
grep -n "imdbRating\|rating\|score\|struct LibraryItem" \
  /Users/zain/projects/Moonlit/Packages/MoonlitCore/Sources/MoonlitCore/Services/LibraryRepository.swift | head -10
```
...
grep -n "func progress\|func watchProgress" \
  /Users/zain/projects/Moonlit/Packages/MoonlitCore/Sources/MoonlitCore/Services/WatchProgressRepository.swift | head -10
```
...
```bash
git add Apps/MoonlitApp/Sources/Screens/LibraryScreen.swift
