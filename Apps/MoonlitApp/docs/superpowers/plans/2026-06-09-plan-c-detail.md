...
**Architecture:** `DetailScreen.swift` gets three additions: `DescriptionSheet`, portrait `CastCard`, and a `NavigationLink` to `ActorBioScreen`. `ActorBioScreen.swift` is a new file with its own `ActorBioViewModel` that calls `TMDBPersonService` (a new MoonlitCore service). Liked status is managed by a new `LikedRepository` (used by both Plan C and Plan D).

**Tech Stack:** SwiftUI, TMDB Person API, MoonlitCore, URLSession
...
| Action | Path |
|---|---|
| Create | `Packages/MoonlitCore/Sources/MoonlitCore/Services/TMDBPersonService.swift` |
| Create | `Packages/MoonlitCore/Sources/MoonlitCore/Services/LikedRepository.swift` |
| Create | `Packages/MoonlitCore/Sources/MoonlitCore/Models/PersonModels.swift` |
| Modify | `Apps/MoonlitApp/Sources/Screens/DetailScreen.swift` |
| Create | `Apps/MoonlitApp/Sources/Screens/ActorBioScreen.swift` |
...
```swift
// Packages/MoonlitCore/Sources/MoonlitCore/Models/PersonModels.swift
import Foundation
...
```bash
git add Packages/MoonlitCore/Sources/MoonlitCore/Models/PersonModels.swift
...
grep -rn "tmdbApiKey\|TMDBService\|MetadataIntegration" \
  /Users/zain/projects/Moonlit/Packages/MoonlitCore/Sources/MoonlitCore/ | head -20
```
...
```swift
// Packages/MoonlitCore/Sources/MoonlitCore/Services/TMDBPersonService.swift
import Foundation
...
```bash
git add Packages/MoonlitCore/Sources/MoonlitCore/Services/TMDBPersonService.swift
...
grep -n "struct LibraryItem\|class LibraryRepository\|func addToLibrary\|func loadLibrary" \
  /Users/zain/projects/Moonlit/Packages/MoonlitCore/Sources/MoonlitCore/Services/LibraryRepository.swift | head -20
```

- [ ] **Step 1: Create LikedRepository mirroring LibraryRepository**

```swift
// Packages/MoonlitCore/Sources/MoonlitCore/Services/LikedRepository.swift
import Foundation
...
public final class LikedRepository: ObservableObject {
    ...
    private let storageKey = "moonlit.liked.items"
...
```bash
git add Packages/MoonlitCore/Sources/MoonlitCore/Services/LikedRepository.swift
...
grep -n "synopsis\|description\|cast\|CastCard\|person\." \
  /Users/zain/projects/Moonlit/Apps/MoonlitApp/Sources/Screens/DetailScreen.swift | head -40
```
...
```swift
// Apps/MoonlitApp/Sources/Screens/ActorBioScreen.swift
import SwiftUI
import MoonlitCore
...
struct ActorBioScreen: View {
    ...
    private func infoRow(label: String, value: String, isLink: Bool = false) -> some View {
        HStack(alignment: .top, spacing: 12) {
            ...
            Text(value)
                .font(.caption)
                .foregroundColor(isLink ? MoonlitTheme.accent : .white)
                .frame(maxWidth: .infinity, alignment: .leading)
        }
                ...
                ForEach(CreditFilter.allCases, id: \.self) { filter in
                    Button {
                        ...
                        Text(filter.rawValue)
                            ...
                            .padding(.vertical, 6)
                            .background(creditsFilter == filter ? MoonlitTheme.accent : Color.white.opacity(0.08))
                            .cornerRadius(20)
                    }
                }
            }
...
```bash
git add Apps/MoonlitApp/Sources/Screens/ActorBioScreen.swift
