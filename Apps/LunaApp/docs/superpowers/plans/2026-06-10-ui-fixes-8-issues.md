...
## Task 8: Red Accent Verification + LunaTheme Check

**Files:**
- Possibly: `Sources/Screens/LibraryScreen.swift`, any file defining `LunaTheme.accent`

**Research finding:** The red `Color(red:1, green:0.25, blue:0.35)` in LibraryScreen is only used for the heart.fill icon tint. Season pills use `LunaTheme.accent`. The user saw red in screenshots — this may be because `LunaTheme.accent` itself is red, or because the icon tint leaked via some parent modifier.

- [ ] **Step 1: Check LunaTheme.accent definition**

Find `LunaTheme.swift` and read the `accent` color definition. If it is red (or close to the heart tint), that explains why season pills appear red. The accent should be a blue/teal or brand color, not red.

If `LunaTheme.accent` is defined as red, change it to the intended brand color (e.g., a blue `Color(red: 0.2, green: 0.5, blue: 1.0)` or wherever the original brand color was).
...
```bash
git add Sources/Screens/LibraryScreen.swift Packages/LunaCore/Sources/LunaCore/Theme/LunaTheme.swift
git commit -m "fix: scope heart icon tint to image only; verify LunaTheme.accent is correct brand color"
```
