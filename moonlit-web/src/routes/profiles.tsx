  ...
  return (
    <div className="relative flex items-center justify-center min-h-screen overflow-hidden">
      ...
      <div className="relative text-center max-w-lg w-full px-6">
        ...
        <p className="text-sm text-moonlit-muted mb-10">Select a profile to continue</p>
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
            ...
            <button key={p.id} onClick={() => handleSelect(p)}
              ...
              {p.id !== currentProfile?.id && (
                <button onClick={e => { e.stopPropagation(); deleteProfile(p.id); }}
                  className="absolute top-2 right-2 text-xs text-moonlit-muted hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all duration-200 cursor-pointer"
          ...
          <button onClick={() => setShowCreate(true)}
            ...
            <div className="w-16 h-16 rounded-full border-2 border-dashed border-white/20 hover:border-moonlit-accent/50 flex items-center justify-center text-2xl text-moonlit-muted hover:text-moonlit-accent transition-all duration-200">+</div>
            <span className="text-sm text-moonlit-muted hover:text-white transition-colors">Add Profile</span>
          </button>
        </div>
        ...
        <button onClick={signOut} className="text-sm text-moonlit-muted hover:text-white transition-colors cursor-pointer">
          Sign Out
        </button>
      </div>
    </div>
